...
function renderDashboard(data) {
  ...
  if (sortedCli.length > 0) {
    ...
    if (topPct > 30) {
      const nivel = topPct > 40 ? 'warning' : 'warning';
      alertHTML += `<div class="alert ${nivel}"><span class="icon">!</span><div><strong>Concentración de cliente.</strong> ${sortedCli[0][0]} representa el ${fmt(topPct)} % de toda la facturación.${topPct < 40 ? ' Por debajo del umbral crítico del 40 %, pero conviene diversificar.' : ' ¡Supera el umbral crítico del 40 %!'}</div></div>`;
    }
  }
  // IRPF
  if (irpf > 0) {
